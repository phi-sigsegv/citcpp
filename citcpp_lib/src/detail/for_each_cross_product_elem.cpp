...
/**
 * @brief Recursive function to generate the cartesian product of multiple
 * zero-based integer ranges.
 *
 * @param max_values A vector where each element is the maximum value for a
 * range (min is implicitly 0). E.g., {2, 1, 5} means ranges [0,2], [0,1],
 * [0,5].
 * @param current_index The index of the range currently being processed.
 * @param callback_data The vector storing the cross product element being
 * built.
 * @param callback The callback to be called for each cross product element.
 */
void recursive_cross_product(
    const std::vector<unsigned int> &max_values, unsigned int current_index,
    std::vector<unsigned int> &callback_data,
    const std::function<void(const std::vector<unsigned int> &)> &callback) {
...
void for_each_cross_product_elem(
    const std::vector<unsigned int> &max_values,
    const std::function<void(const std::vector<unsigned int> &)> &callback) {
