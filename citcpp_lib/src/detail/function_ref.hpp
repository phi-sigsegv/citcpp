...
namespace citcpp
{
  namespace detail
  {
    ...
    template<typename Ret, typename ...Params>
      ...
      private:
	Ret
	(*callback) (intptr_t callable, Params ...params) = nullptr;
	intptr_t callable;

	template<typename Callable>
	  static Ret
	  callback_fn (intptr_t callable, Params ...params)
	  {
	...
	template<typename Callable>
	  function_ref (
	      ...
	      callback (
		  callback_fn<typename std::remove_reference<Callable>::type>), callable (
