 ...
 */
static Matrix backsub(int depth, Matrix a, Matrix l)
{
    if (depth == BLOCK_DEPTH) {
        LeafNode *A = (LeafNode *) a;
 ...
 */
static Matrix cholesky(int depth, Matrix a)
{
    if (depth == BLOCK_DEPTH) {
        LeafNode *A = (LeafNode *) a;
...
int main( int argc, char **argv )
{
    if (argc == 1) {
        ...
        n = atoi(argv[1]);
        nonzeros = atoi(argv[2]);
    }

    printf("Running cholesky n=%d nonzeros=%d sequentially...\n", n, nonzeros);
