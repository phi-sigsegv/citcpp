 ...
 *
 */
void add_matrix(block * T, long ot, block * R, long oR, long x, long y)
{
    if (x + y == 2) {
        long i;
    ...
    if (x > y) {
        add_matrix(T, ot, R, oR, x/2, y);
        add_matrix(T+(x/2)*ot, ot, R+(x/2)*oR, oR, (x+1)/2, y);
    } else {
        add_matrix(T, ot, R, oR, x, y/2);
...
static void multiply_matrix(block * A, long oa, block * B, long ob,
...
int main(int argc, char **argv)
{
    ...
    printf("Running rectmul n=%d sequentially...\n", n);
