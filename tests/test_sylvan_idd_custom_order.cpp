...
TEST_CASE("constraint handler sylvan IDD, testing custom variable order") {
    ...
    auto prop1 = std::make_shared<boolean_proposition>(parameter_reference("P1"), relational_operator::EQ, true);
    auto prop3 = std::make_shared<boolean_proposition>(parameter_reference("P3"), relational_operator::EQ, false);
    ...
    SUBCASE("is_valid_partial_test") {
        test t1({1, -1, 1}); // P1=false, P2=dont_care, P3=false -> valid (premise false)
        ...
        test t_invalid({0, -1, 0}); // P1=true, P3=true -> invalid (violates implication)
    ...
    SUBCASE("mark_valid_tuples") {
        ...
        coverage_map_second_level cov(4, {0, 2});
        handler.mark_valid_tuples(cov);
